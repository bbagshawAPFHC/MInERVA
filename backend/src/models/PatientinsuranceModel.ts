...
interface patientinsurance__id_Buffer {
  0: any;
  1: any;
  2: any;
  3: any;
...
const patientinsurance__id_BufferSchema: Schema = new Schema({
...
interface patientinsurance__id {
  buffer: patientinsurance__id_Buffer;

}

const patientinsurance__idSchema: Schema = new Schema({
  buffer: { type: patientinsurance__id_BufferSchema, required: true },

});

interface patientinsurance_Patientdetails {
  state: any;
  lastname: any;
  firstname: any;
  homephone: any;
...
const patientinsurance_PatientdetailsSchema: Schema = new Schema({
...
interface Patientinsurance extends Document {
  _id: patientinsurance__id;
  insurances: any;
  totalcount: any;
  patientdetails: patientinsurance_Patientdetails;

}

const PatientinsuranceModelSchema: Schema = new Schema({
  _id: { type: patientinsurance__idSchema, required: true },
  insurances: { type: Schema.Types.Mixed, required: true },
  totalcount: { type: Schema.Types.Mixed, required: true },
  patientdetails: { type: patientinsurance_PatientdetailsSchema, required: true },

});

const PatientinsuranceModel = mongoose.model<Patientinsurance>('patientinsurance', PatientinsuranceModelSchema, 'patientinsurance');
