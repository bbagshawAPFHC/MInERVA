...
interface claimtransactions__id_Buffer {
  0: any;
  1: any;
  2: any;
  3: any;
...
const claimtransactions__id_BufferSchema: Schema = new Schema({
...
interface claimtransactions__id {
  buffer: claimtransactions__id_Buffer;

}

const claimtransactions__idSchema: Schema = new Schema({
  buffer: { type: claimtransactions__id_BufferSchema, required: true },

});

interface claimtransactions_Patientdetails {
  state: any;
  lastname: any;
  firstname: any;
  homephone: any;
...
const claimtransactions_PatientdetailsSchema: Schema = new Schema({
...
interface Claimtransactions extends Document {
  _id: claimtransactions__id;
  totalcount: any;
  claimtransactions: any;
  patientdetails: claimtransactions_Patientdetails;

}

const ClaimtransactionsModelSchema: Schema = new Schema({
  _id: { type: claimtransactions__idSchema, required: true },
  totalcount: { type: Schema.Types.Mixed, required: true },
  claimtransactions: { type: Schema.Types.Mixed, required: true },
  patientdetails: { type: claimtransactions_PatientdetailsSchema, required: true },

});

const ClaimtransactionsModel = mongoose.model<Claimtransactions>('claimtransactions', ClaimtransactionsModelSchema, 'claimtransactions');

export default ClaimtransactionsModel;