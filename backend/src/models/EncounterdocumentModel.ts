...
interface encounterdocument__id_Buffer {
  0: any;
  1: any;
  2: any;
  3: any;
...
const encounterdocument__id_BufferSchema: Schema = new Schema({
...
interface encounterdocument__id {
  buffer: encounterdocument__id_Buffer;

}

const encounterdocument__idSchema: Schema = new Schema({
  buffer: { type: encounterdocument__id_BufferSchema, required: true },

});

interface encounterdocument_Patientdetails {
  state: any;
  lastname: any;
  firstname: any;
  homephone: any;
...
const encounterdocument_PatientdetailsSchema: Schema = new Schema({
...
interface Encounterdocument extends Document {
  _id: encounterdocument__id;
  encounterdocuments: any;
  totalcount: any;
  patientdetails: encounterdocument_Patientdetails;

}

const EncounterdocumentModelSchema: Schema = new Schema({
  _id: { type: encounterdocument__idSchema, required: true },
  encounterdocuments: { type: Schema.Types.Mixed, required: true },
  totalcount: { type: Schema.Types.Mixed, required: true },
  patientdetails: { type: encounterdocument_PatientdetailsSchema, required: true },

});

const EncounterdocumentModel = mongoose.model<Encounterdocument>('encounterdocument', EncounterdocumentModelSchema, 'encounterdocument');

export default EncounterdocumentModel;